...
# Каждые 10 минут, а не раз в час: перенос задачи в другой проект виден
# отчётам только после обновления справочника, и часовое отставание читается
# как «приложение не заметило перенос» (жалоба 31.08.2026). Прогон стоит ~17
# секунд на 1 600 задач — десятиминутный ритм ему по силам.
