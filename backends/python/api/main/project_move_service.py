...
class ProjectMoveService:
    ...
    def _mirror_locally(self, bitrix_ids: List[Any], new_group: str,
        ...
        ПРАВИМ ВСЕ КОПИИ КАРТОЧКИ НА ПОРТАЛЕ, А НЕ ТОЛЬКО СВОЮ.

        Одна карточка списания лежит в нашей базе НЕСКОЛЬКИМИ строками — по
        одной на аккаунт, которым её увидел синк (на проде 31.08.2026: 25 239
        строк на 7 149 карточек, то есть в среднем 3,5 копии, 31 аккаунт).

        Первая версия зеркала правила только копию текущего аккаунта, и этого
        оказалось мало: выравнивание запускается от РАЗНЫХ аккаунтов
        (в логе — свой account на каждом прогоне), поэтому следующий прогон
        видел ЧУЖУЮ, ещё не поправленную копию, считал задачу разошедшейся и
        переписывал те же карточки заново, снова оставляя комментарий. Ровно
        столько раз, сколько аккаунтов держат копию.

        Граница — домен портала, а не аккаунт: карточка в Битриксе одна, и все
        её копии в пределах портала описывают один и тот же объект. Шире брать
        нельзя — идентификаторы карточек уникальны внутри портала, но не между
        порталами.
        """
        if not bitrix_ids:
            return
        ...
        try:
            TimesheetItem.objects.filter(
                bitrix24_account__domain_url=self.account.domain_url,
