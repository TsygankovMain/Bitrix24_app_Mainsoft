...
export const useAppInit = (loggerTitle?: string) => {
  ...
  async function initApp(
    ...
    // Состояния платных функций портала (GET /api/features, контракт «Счёта и
    // акта»). Спрашиваем ЗДЕСЬ, потому что это единственная точка приложения,
    // после которой гарантированно есть JWT, а состояние подписки нужно
    // постоянному меню — оно рисуется в лейауте раньше и своего запроса
    // сделать не может. Дедупликация и кэш внутри: на навигации повторного
    // запроса не будет.
    //
    // Отказ НЕ пробрасываем и не логируем ошибкой: пока бэкенд функции не
    // поднят, ручка отвечает 404, и валить из-за неё вкладку задачи или
    // отчёты нельзя. useBillingFeature запомнит «не ответила» и оставит
    // функцию закрытой — замок вместо ложного обещания.
    await useBillingFeature().loadPortalFeatures()
