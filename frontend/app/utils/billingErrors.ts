...
const ACT_TEXTS: Record<string, { title: string, text: string }> = {
  act_template_missing: {
    title: 'На портале нет шаблона акта',
    text: 'Генератор документов не нашёл шаблон, по которому печатать акт. Добавьте шаблон акта в настройках '
      + 'генератора документов портала — или выгрузите детализацию в XLSX и оформите акт по ней.',
  },
...
export function describeBillingError(error: unknown): BillingErrorView {
  ...
  if (ACT_TEXTS[code]) {
    return { ...ACT_TEXTS[code], code, documentId: '', permanent: false }
  }
