 ...
 *
 * Состояние приходит ТОЛЬКО с фронта (FINANCE_* в featureFlags.ts). Бэкенд о
 * подписке ничего не знает, и делать вид, что знает, нельзя: замок здесь —
 * витрина, а не защита. Ничего платного за ним пока не стоит, поэтому и
 * защищать нечего; когда функция появится, право доступа будет проверять
 * сервер, а флаг останется управлять только точкой входа.
 *
