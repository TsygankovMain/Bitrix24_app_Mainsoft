...
/**
 * Причина, по которой «Выставить» погашена нулевой суммой при непустом
 * документе (Баг 9).
 *
 * До этой правки issuable гасил кнопку молча: человек видел плашку no_rate
 * «выставить можно и так — счёт просто не досчитает эти часы» и тут же
 * немую погашенную кнопку, что противоречит плашке. Разбор: счёт на 0 ₽ в
 * CRM бессмысленен (сверять с оплатой нечего), поэтому блокировка
 * остаётся — но с объяснением рядом с кнопкой вместо немого отказа.
 * Бэкенд отклоняет такой документ тем же кодом (ERROR_ZERO_AMOUNT в
 * billing_service.py::validate_for_issue) — эта кнопка лишь опережает его
 * своей проверкой, чтобы не тратить запрос на заведомый отказ.
 *
 * `null`, если строк нет (тогда действует emptyReason.ts) или сумма уже
 * больше нуля.
 */
export function describeZeroAmountBlock(totals: BillingTotals): string | null {
  if (totals.linesCount > 0 && totals.totalAmount <= 0) {
    return 'Сумма счёта 0 ₽: у всех строк нет ставки — задайте ставку в карточке проекта.'
  }
