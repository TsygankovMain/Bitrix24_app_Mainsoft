...
export interface ShouldOfferCompanyCreationInput {
  ...
  isSearching: boolean
  /** Сколько вариантов сейчас показано в открытом списке (visibleOptions.length в SearchableSelect.vue). */
  optionCount: number
}

/**
 * Д2 хотфикса 2026-07-29 (см. .superpowers/sdd/2026-07-28-create-project-button/
 * hotfix-new-company-brief.md): решает, показывать ли в открытом списке
 * SearchableSelect действие "Создать компанию «...»" вместо надписи "Ничего
 * не найдено". Раньше этого действия не существовало вообще — надпись была
 * неактивной (@click="closeDropdown"), хотя подсказка под полем в
 * CreateProjectModal.vue обещала обратное.
 *
 ...
 *  - isSearching===false — ответ текущего запроса ещё не пришёл. serverResults
 *    в SearchableSelect.vue::runServerSearch НЕ очищаются в момент старта
 *    нового поиска (это отдельное намеренное решение — старый список не
 *    должен мигать в пустоту на время запроса), поэтому optionCount===0 в
 *    момент isSearching===true может значить "ещё не знаем", а не "точно
 *    ничего нет". Показать действие раньше времени — предложить создать то,
 *    что через мгновение найдётся;
 *  - optionCount === 0 — сервер поискал и не нашёл ВООБЩЕ ничего.
 *
 * Частичное совпадение — сервер вернул варианты (optionCount > 0), но ни
 * один из них не совпадает с query дословно — СОЗНАТЕЛЬНО не считается
 * поводом предложить создание: при optionCount > 0 функция всегда false,
 * даже если ни один вариант не идентичен запросу. Причины:
 *  1. Сигнатура намеренно получает только optionCount, а не сами варианты.
 *     "Похож ли вариант на query" — вопрос нечёткого сравнения (регистр,
 *     кавычки, "ООО"/"АО" и другие организационно-правовые формы, филиалы
 *     одной компании с разными ИНН), и его правильный ответ требует данных,
 *     которых у этой функции по построению нет.
 *  2. Асимметрия цены ошибки. Ошибка "не предложили создание, хотя человек
 *     хотел создать" стоит пару секунд: он либо уточняет запрос, пока
 *     optionCount не станет нулём (тогда действие появится само), либо жмёт
 *     "Сбросить". Ошибка "предложили создание, хотя нужная компания уже была
 *     в списке" стоит дубля в справочнике портала — а дубль расползается
 *     дальше (см. докстринг companyFieldsForQuery выше: CompanySearchService
 *     дописывает company_name туда, где находит company_id) и обратной
 *     кнопки "удалить компанию из Битрикса" у формы нет. Вторая ошибка
 *     дороже первой на порядок, поэтому при малейшей неопределённости
 *     (optionCount > 0) функция отказывается предлагать создание.
 *  3. Ничего не потеряно: список найденных вариантов в этот момент и так на
 *     экране. Человек видит, что похожие компании есть, и решает сам —
 *     выбрать одну (если это опечатка) или уточнить запрос настолько, чтобы
 *     он точно ни с чем не совпадал.
 */
export function shouldOfferCompanyCreation({ query, isSearching, optionCount }: ShouldOfferCompanyCreationInput): boolean {
  if (isSearching) {
    return false
  }
