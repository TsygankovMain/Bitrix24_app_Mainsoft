...
const totals = computed(() => {
  ...
  return [
    { id: 'focus', label: 'Индекс фокуса', value: formatIndex(summary.avg_focus_index), tone: 'info' as const },
    { id: 'entry-size', label: 'Ср. размер записи', value: `${summary.avg_entry_size.toFixed(2)}ч`, tone: 'success' as const },
    { id: 'entries-per-employee', label: 'Ср. записей / сотрудник', value: summary.avg_entries_per_employee },
    { id: 'switchers', label: '5+ проектов', value: summary.high_switch_employee_count, tone: 'warning' as const },
    { id: 'risk', label: 'Высокий риск', value: summary.high_risk_employee_count, tone: 'danger' as const },
    { id: 'employees', label: 'Сотрудников', value: summary.employee_count },
  ]
})
...
<template>
  <ReportShell
    v-if="isInit"
    title="Фокус и распыление"
    description="Как сотрудники распределяют часы между проектами и задачами"
    :date-from="dateFrom"
    :date-to="dateTo"
