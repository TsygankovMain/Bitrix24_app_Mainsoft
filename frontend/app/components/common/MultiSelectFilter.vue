...
<template>
    <div ref="rootRef" class="relative inline-block w-full max-w-[320px] text-left">
        ...
        <!--
            ВНИМАНИЕ при переносе этого фильтра в новое место.

            Список нарисован через position:absolute внутри якоря. Это работает
            ровно до тех пор, пока НИ ОДИН предок не обрезает содержимое.
            Положишь фильтр в модальное окно, сайдбар или любой контейнер с
            overflow-hidden / overflow-y-auto / фиксированной высотой — список
            молча обрежется по границе этого контейнера, и до отрезанного
            нельзя будет доскроллить. Именно этот дефект 29.07.2026 сделал
            непригодной форму «Создать проект».

            Проверено 29.07.2026 по всем восьми потребителям (InnBackfillPanel
            и семь отчётных страниц): ни один не рисует фильтр внутри
            обрезающего контейнера, поэтому здесь оставлен простой absolute.
            Найденные overflow-hidden в revenue-leakage и time-discipline —
            декоративные полоски прогресса высотой в несколько пикселей, а
            модалка в daily.client.vue лежит в соседней ветке разметки, не над
            фильтром.

            Если условие изменится — не чините подгонкой отступов и z-index.
            Решение уже готово в SearchableSelect.vue: телепорт панели в body,
            position:fixed, координаты через computeDropdownPosition
            (app/utils/dropdownPosition.ts, покрыта тестами), обработка клика
            вне списка с учётом телепортированной панели. Там же в докстрингах
            описана ловушка с захватом фокуса у диалогов reka-ui/B24Modal: без
            её обхода в модальном окне в поле поиска нельзя будет печатать.
        -->
        <div
            v-if="isOpen"
