...
<template>
  <template v-if="node">
      ...
      <tr :class="['cursor-pointer border-b transition-colors', rowClass]" @click="toggle">
        ...
        <!-- Hours Columns -->
        <td class="px-6 py-2 whitespace-nowrap text-sm font-medium text-slate-900">
        ...
        <td class="px-6 py-2 whitespace-nowrap text-sm text-slate-400">
            {{ typeLabel }}
        </td>
      </tr>
      ...
      <template v-if="isOpen && node.items">
          <tr v-for="item in node.items" :key="item.id_elem" class="border-b bg-white hover:bg-slate-50">
               <td class="flex items-start py-2 pr-4 text-sm text-slate-600" :style="{ paddingLeft: `${(level + 1) * 20 + 20}px` }">
               ...
               <td class="px-6 py-2 whitespace-nowrap text-sm text-slate-900">
