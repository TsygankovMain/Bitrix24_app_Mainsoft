...
<template>
  <B24Modal :open="visible" @update:open="(v) => { if (!v) emit('close') }">
    <template #header>
      <div>
        <div class="text-base font-semibold text-slate-900">Заполнение ИНН · «{{ projectName }}»</div>
        <p class="text-xs text-slate-500 mt-1">Применится ко всем карточкам проекта за период</p>
      </div>
    </template>
    ...
    <template #body>
      <div class="space-y-4">
        <div>
          ...
          <input v-model="localOur" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="наш ИНН">
        </div>
        <div>
