...
Пользовательские релизы (человеческим языком) — в [RELEASES.md](./RELEASES.md).
...
#### S2 — каркас страниц на B24 (Changed)
- Главная (`pages/index.client.vue`), настройки (`settings/index`, `settings/mapping`, `settings/projects-health`), отчёты (`reports/raw-data`, `reports/guide`) переведены на `B24Container`/`B24PageHeader`/`B24Card`/`B24PageGrid`/`B24Empty`. Эталон — `projects-health` (единая шапка через `#links`, без дублей).
- Уплотнение «слишком воздушного» Air через `app.config.ts` (`b24ui`-оверрайды паддингов карточек/шапок/гэпов) — по фидбеку пользователя.

#### S3 — модалки/тосты/формы (Changed)
- `HelpModal` → `B24Modal` (`:open`/`close`); `HelpSidePanel` → `B24Slideover`; `InnAssignModal` → `B24Modal` (логика `apply()` сохранена). `alert()`/самопальные оверлеи → `useToast`. Лайм в селектах/тогглах/иконках вычищен на нативный синий.

#### S4 — таблицы отчётов: ре-скин УТП (Changed, высокий риск)
- Рекурсивные компоненты (`ProjectTaskReportRow`, `ProjectTaskReportEmployeeRow`, `RecursiveTableRow`, `InnBackfillPanel`, `daily.client.vue`): лайм → нативные нейтрали/синий. **Рекурсия, раскрытие/сворачивание, метки, структура Excel — НЕ тронуты** (только цвет). Замена на `B24Table` сознательно НЕ делалась (защита УТП).

#### S5 — экран отражения времени + чистка (Changed)
- `embedded.vue` + `task.vue`/`TaskNode`/`TaskGroupComponent`/`TaskItemRow`: лайм → синий (14 цвет-свопов, push `1724667`); `alert()` → тосты `useToast` (12 алертов, push `923e733`). Логика отражения (`crm.item.add`, резолв ИНН, иерархия, разделение, логи `[Embedded][INN]`) — идентична.
- **Тест в реальном iframe Bitrix подтверждён пользователем**: отражение времени, простановка ИНН, иерархия, Split, ошибки-тосты — работают.
- Чистка `main.css`: удалены 4 неиспользуемых `ms-*` (`ms-card-grid`, `ms-helper-text`, `ms-segmented-btn-active-lime`, `ms-surface-muted`) + переменная `--ms-surface-muted` (push `3bbe9bb`). `nuxt build` EXIT=0.

#### Отложено (отдельный будущий спринт)
- **Тёмная тема.** Замер: ~750+ зашитых светлых утилит (`bg-white`/`text-slate-*`/`border-slate-200`) в 42 файлах, `dark:`-вариантов 0. Включение `colorMode: true` сейчас дало бы «полу-тёмный» сломанный UI. Подход на будущее — централизация цветов в семантические CSS-токены (`ms-*` переменные с тёмными значениями), а не россыпь `dark:`-вариантов. **Решение пользователя 2026-06-01: закрыть миграцию на светлой теме.**

### Миграция на B24 UI Kit — S0/S1 — 2026-06-01

#### Docs
- Оценка, план спринтов и смоук-чек-лист миграции лежали в `docs/archive/`. Каталог удалён после завершения миграции — документы доступны в истории git.
