...
### 4. Миграции БД (применяются автоматически при старте)

> ✅ `start.sh` прогоняет `python manage.py migrate --noinput` при старте контейнера — отдельный ручной релизный шаг НЕ нужен. При ошибке миграции контейнер не поднимется (`set -e`): это видно сразу в логе деплоя, вместо **500 на каждом запросе к моделям** (`TimesheetItem`/`ProjectCard`) при отсутствующей колонке.
>
> Так сделано потому, что ручной шаг пропускали: 2026-07-27 прод-БД оказалась на `0011` при коде `0015` — таблицы `sync_run` не существовало, и фоновый синк проектов молча падал на каждой итерации.
...
**Если `migrate` падает с `DuplicateColumn: column ... already exists` или `DuplicateTable: relation ... already exists`** — это расхождение лайнеджей миграций (схема уже содержит объект, который миграция пытается создать; типично после merge ветки с другой историей миграций). Реальный случай 2026-07-27: `0014_portal_and_nullable_fk` упала на `relation "portal" already exists`, при этом колонки `portal_id` во всех трёх таблицах (`bitrix24account`, `project_card`, `timesheet_item`) уже существовали → миграция помечена `--fake`, дальше `migrate` доехал штатно. Порядок реконсиляции:
...
8. На странице отчёта: открытие НЕ шлёт `POST /api/sync-timesheets` (данные
   читаются из БД) — в `Network` его быть не должно; виден индикатор «данные
   на ЧЧ:ММ». Кнопка «Обновить» инициирует scoped `POST /api/sync-timesheets`
   → `200`, в консоли нет 5xx. Тихий 500 на синке не ломает страницу визуально
   — см. [инцидент 2026-06-10](./incidents/2026-06-10-sync-advisory-lock-bigint.md).
