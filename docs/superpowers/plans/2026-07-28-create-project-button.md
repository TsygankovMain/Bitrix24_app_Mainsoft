    ...
    def ensure_card(
        ...
        *,
        ...
        if group_field:
            ...
            if existing:
                # Здесь, в отличие от компании и группы, несколько найденных
                # карточек НЕ дают ambiguous, а берётся первая. Это осознанно:
                # шаги 1 и 2 при неопределённости отказываются СОЗДАВАТЬ, потому
                # что лишняя компания или группа останется на портале навсегда.
                # Здесь же мы, наоборот, ничего не создаём — переиспользуем уже
                # существующее. Худшее последствие выбора не той из двух карточек
                # одной группы — карточка привязана не к той записи, это видно
                # в интерфейсе и правится вручную; необратимого мусора не
                # появляется. Спрашивать пользователя про ситуацию, которой в
                # штатной работе быть не должно, — цена выше пользы.
                return StepResult(
                    status="found",
...
### Task 6: HTTP-эндпоинт создания проекта
...
## Шаги 6-13 выполнены вне этого плана — эндпоинты уже в проде

Изначально задача 6 включала создание двух эндпоинтов: поиска компаний по мере ввода и списка своих юрлиц. **Они реализованы и выкачены в прод 2026-07-28** в составе варианта Б (`docs/superpowers/plans/2026-07-28-project-references-from-db.md`, задачи 1 и 5), потому что от них зависела скорость экрана проектов, а не только эта форма.

Что уже существует и чем нужно пользоваться, а не писать заново:

- `backends/python/api/main/company_search_service.py` — класс `CompanySearchService`:
  - `search(query, limit)` → `{"companies": [{"id", "name", "inn"}], "truncated": bool, "failed": bool}`
  - `list_my_companies(bypass_cache=False)` → `{"companies": [{"id", "name"}], "failed": bool}`
- `GET /api/project-board/companies/search?q=<строка>&limit=50` — под ограничителем 60 запросов в минуту на аккаунт.
- `GET /api/project-board/my-companies` — список своих юрлиц серверным фильтром.
- Фронт: `useApiStore().searchCompanies(query, limit)` и `useApiStore().getMyCompanies()` уже есть в `frontend/app/stores/api.ts`.
- `SearchableSelect.vue` уже умеет режим серверного поиска — необязательный проп с функцией поиска, задержка ввода, три состояния (нашлось / Битрикс недоступен / слишком часто спрашиваем).

**Семантика, которую нельзя перепутать:** признак `failed` означает «не доверяй полноте списка», а **не** «список пуст» — список может быть непустым и при `failed`. Признак `truncated` означает «совпадений больше, чем показано, уточните запрос».

Сервис прошёл три круга ревью: защищён от неожиданных форм ответа Битрикса, различает «ничего не нашлось» и «ответ не разобран», не кэширует неудачи.

Таким образом задача 6 сводится к шагам 1-5 выше — только эндпоинт создания проекта.
