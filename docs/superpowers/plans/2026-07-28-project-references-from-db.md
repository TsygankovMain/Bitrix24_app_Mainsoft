...
### Task 4: Сотрудники в meta из portal_user + рабочая кнопка «Обновить справочники»
...
- [ ] **Step 6: Починить кнопку «Обновить справочники»**

Найдено при ре-ревью Task 2, подтверждено трассировкой всей цепочки. В интерфейсе есть кнопка «Обновить справочники» (`frontend/app/pages/projects/index.client.vue`, обработчик `refreshReferenceOptions`). Она вызывает `loadMeta(true)` → `apiStore.getProjectBoardMeta(true)`, но признак `forceRefresh` там управляет **только браузерным кэшем**: сам HTTP-запрос уходит без каких-либо параметров. Бэкенд про принудительность не знает и отдаёт свой кэш `project-board-meta`, который живёт 6 часов.

То есть кнопка, буквально названная «Обновить справочники», **не обновляет на сервере ничего**. Дефект существовал и до этого плана, но именно на «есть кнопка обновления» я ссылался, поднимая время жизни кэша до 6 часов, — значит обоснование было ложным.

Что сделать:

- `get_meta(self, bypass_cache: bool = False)` — при `True` не читать кэш, а перечитать источники и записать свежее значение. Внутрь пробросить `bypass_cache` в `get_legal_entities` (параметр уже есть после Task 2).
- View `get_project_board_meta`: читать признак из запроса, например `?refresh=1`, и передавать в сервис. Разбор параметра должен быть устойчив к мусору — пустая строка, слово, что угодно не должно приводить к ошибке.
- Фронт: `getProjectBoardMeta(forceRefresh)` должен добавлять этот параметр в URL, когда `forceRefresh` истинно. Сейчас он влияет только на локальный кэш.

Тесты: с параметром кэш игнорируется и данные перечитываются; без параметра кэш работает как раньше; мусор в параметре не роняет эндпоинт.

- [ ] **Step 7: Коммит**

```bash
git add backends/python/api/main/project_board_service.py backends/python/api/main/views.py backends/python/api/main/tests_project_references.py frontend/app/stores/api.ts
git commit -m "fix(references): кнопка «Обновить справочники» действительно обновляет их на сервере"
```

**Известное ограничение, которое этой правкой не закрывается.** Кэш живёт в памяти процесса, а воркеров несколько. Принудительное обновление прогреет кэш только того воркера, который обработал запрос; следующий запрос может попасть на другой. Полное решение — общий кэш на всех воркеров, это отдельная задача. Здесь мы делаем кнопку честной хотя бы в пределах одного воркера, что строго лучше нынешнего «не делает ничего».
...
### Task 6: Дозаполнение ИНН — явный вызов полного обхода + честное предупреждение

**СРОЧНОСТЬ: эта задача выполняется сразу после Task 3, вне исходного порядка.** Пока она не сделана, вся ветка не выкатываема: после Task 3 карта ИНН пустеет для всех карточек.

**Найдено при ревью Task 3, обязательно к исправлению здесь.** Предохранитель в `scan()` (`inn_backfill_service.py`) не срабатывает в этом состоянии:

```python
if with_project > 0 and not (companies_inn or legal_inn):
```

Условие проверяет пустоту словаря целиком, а словари теперь непустые — ключи есть, а значения пустые строки. Ревьюер подтвердил прогоном: `scan()` отдаёт `warning: None` при `resolvable: 0`. То есть администратор видит обычный экран со строками «требует внимания» и **ни одного намёка**, что источник данных сломан.

Баг существовал и раньше — для редкого случая «Битрикс отвечает, но ИНН не заведён ни у одной компании». Task 3 превратила его из теоретического в гарантированный.

Здесь нужно поправить условие так, чтобы оно ловило и «словарь пуст», и «в словаре одни пустые значения». Тест на оба случая.
