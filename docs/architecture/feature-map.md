...
- **Фронт:** `pages/embedded.vue` — placement в карточке задачи (экран `task.vue` удалён в редизайне, вариант A); `composables/useTaskPlacement.ts`, `composables/useTaskTreeLoader.ts`; компоненты `components/TaskGroupComponent.vue`, `TaskNode.vue`, `TaskItemRow.vue`.
